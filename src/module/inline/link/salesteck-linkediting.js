...
import {
	Input, TwoStepCaretMovement,
	// inlineHighlight, findAttributeRange
...
// import {
// 	// createLinkElement, ensureSafeUrl,
// 	// getLocalizedDecorators, normalizeDecorators
// } from './utils';

import './theme/link.css';
import {DATA_BLOCK_TYPE} from "../../../const";
import InlineElementEditing from "../element/inline-element-editing";
 ...
 */
export default class SalesteckLinkEditing extends Plugin {
	/**
	 * @inheritDoc
	 */
	static get pluginName() {
		return 'SalesteckLinkEditing';
	}
	...
	init() {
		...
		editor.conversion.for('upcast').add( dispatcher =>{
			dispatcher.on('element:a', (evt, data, conversionApi) =>{
				...
				const dataBlockTypeAttr = viewItem.getAttribute('data-block-type');
				// console.log(`${evt.name}`, {data, dataBlockTypeAttr});
				if(dataBlockTypeAttr === InlineElementEditing.modelName){
					evt.stop();
				}
...
/*
	/!**
	 * Processes an array of configured {@link @ckEditor5/link/link~LinkDecoratorAutomaticDefinition automatic decorators}
	 * and registers a {@link @ckEditor5/engine/conversion/downcastdispatcher~DowncastDispatcher downcast dispatcher}
	 * for each one of them. Downcast dispatchers are obtained using the
	 * {@link @ckEditor5/link/utils~AutomaticDecorators#getDispatcher} method.
	 *
	 * **Note**: This method also activates the automatic external link decorator if enabled with
	 * {@link @ckEditor5/link/link~LinkConfig#addTargetToExternalLinks `config.link.addTargetToExternalLinks`}.
	 *
	 * @private
	 * @param {Array.<@ckEditor5/link/link~LinkDecoratorAutomaticDefinition>} automaticDecoratorDefinitions
	 *!/
	...
	/!**
	 * Processes an array of configured {@link @ckEditor5/link/link~LinkDecoratorManualDefinition manual decorators},
	 * transforms them into {@link @ckEditor5/link/utils~ManualDecorator} instances and stores them in the
	 * {@link @ckEditor5/link/linkcommand~LinkCommand#manualDecorators} collection (a model for manual decorators state).
	 *
	 * Also registers an {@link @ckEditor5/engine/conversion/downcasthelpers~DowncastHelpers#attributeToElement attribute-to-element}
	 * ckeditor5-converter for each manual decorator and extends the {@link @ckEditor5/engine/model/schema~Schema model's schema}
	 * with adequate model attributes.
	 *
	 * @private
	 * @param {Array.<@ckEditor5/link/link~LinkDecoratorManualDefinition>} manualDecoratorDefinitions
	 *!/
	...
	/!**
	 * Starts listening to {@link @ckEditor5/engine/model/model~Model#event:insertContent} and corrects the model
	...
	/!**
	 * Starts listening to {@link @ckEditor5/engine/view/document~Document#event:mousedown} and
	 * {@link @ckEditor5/engine/view/document~Document#event:selectionChange} and puts the selection before/after a link node
	...
	/!**
	 * Starts listening to {@link @ckEditor5/engine/model/model~Model#deleteContent} and {@link @ckEditor5/engine/model/model~Model#insertContent}
	...
	/!**
	 * Starts listening to {@link @ckEditor5/engine/model/model~Model#deleteContent} and checks whether
	 ...
	 * the {@link @ckEditor5/typing/twostepcaretmovement~TwoStepCaretMovement} plugin is active and
...
//
// @params {@ckEditor5/core/editor/editor~Editor} editor
// @returns {Boolean}
