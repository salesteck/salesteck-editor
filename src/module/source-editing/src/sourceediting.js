...
const COMMAND_FORCE_DISABLE_ID = 'SalesteckSourceEditingMode';
...
export default class SourceEditing extends Plugin {
	...
	constructor( editor ) {
		 ...
		 *
		 * @private
		 * @member {@ckeditor5/utils/elementreplacer~ElementReplacer}
		 */
	...
	init() {
		...
		const t = editor.t;
		// console.log(`${SourceEditing.pluginName}:init`);
		...
		editor.ui.componentFactory.add( 'sourceEditing', locale => {
			...
			this.listenTo( buttonView, 'execute', () => {
				// console.log(`${SourceEditing.pluginName}:execute`, {isSourceEditingMode :this.isSourceEditingMode});
		...
		if ( this._isAllowedToHandleSourceEditingMode() ) {
			this.on( 'change:isSourceEditingMode', ( evt, name, isSourceEditingMode ) => {
				// console.log(`${SourceEditing.pluginName}:${evt.name}:${name}:${isSourceEditingMode}`);
	...
	_showSourceEditing() {
		...
		// console.log(`${SourceEditing.pluginName}:_showSourceEditing`, {editingView});
		...
		for ( const [ rootName, domRootElement ] of editingView.domRoots ) {
			const originalData = editor.data.get( { rootName, trim : 'none' } );
			const data = _formatEditing(formatSource( originalData ));
			...
			// console.log(`${SourceEditing.pluginName}:_showSourceEditing`, {rootName, domRootElement, originalData, data});
	...
	_focusSourceEditing() {
		...
		// console.log(`${SourceEditing.pluginName}:_focusSourceEditing`, {domSourceEditingElementWrapper, textarea});
	 ...
	 * for the {@link @ckeditor5/editor-classic/classiceditor~ClassicEditor classic editor}.
	 *
	 * @private
	 * @returns {Boolean}
	 */
	_isAllowedToHandleSourceEditingMode() {
		...
		const editableElements = editor.ui.view.editables;
		if(_isArray(editableElements) && editableElements.length){
			isEditable = editableElements.every( editable =>{
				// console.log(`${SourceEditing.pluginName}:editableElements.every`, {editableElements, editable, editor, _hasExternalElement : editable._hasExternalElement});
				return editable && !editable._hasExternalElement;
			})
		}


		// console.log(`${SourceEditing.pluginName}:_isAllowedToHandleSourceEditingMode`, {editableElements, editor, isEditable});
...
function _formatEditing(data){
	const htmlEntities = {
		"&amp;" : "&",
		"&nbsp;" : " ",
	};
	Object.entries(htmlEntities).map( ([entitySpec, entityStr])=>{
		// const replace = "entitySpec\\d";
		const re = new RegExp(entitySpec,"g");
		// console.log(`_formatEditing`, {entitySpec, entityStr, re});
		data = data.replace(re, entityStr);
	} );

	// console.log(`_formatEditing`, {data, htmlEntities});
	return data;
