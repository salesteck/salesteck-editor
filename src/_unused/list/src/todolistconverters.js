...
/**
 * A model-to-_view ckeditor5-converter for the `listItem` model element insertion.
 *
 * It converts the `listItem` model element to an unordered list with a {@link @ckEditor5/engine/_view/uielement~UIElement checkbox element}
 * at the beginning of each list item. It also merges the list with surrounding lists (if available).
 *
 * It is used by {@link @ckEditor5/engine/controller/editingcontroller~EditingController}.
 *
 ...
 * A model-to-_view ckeditor5-converter for the `listItem` model element insertion.
 *
 * It is used by {@link @ckEditor5/engine/controller/datacontroller~DataController}.
 *
 * @see module:engine/conversion/downcastdispatcher~DowncastDispatcher#event:insert
 * @param {@ckEditor5/engine/model/model~Model} model Model instance.
 * @returns {Function} Returns a conversion callback.
...
/**
 * A _view-to-model ckeditor5-converter for the checkbox element inside a _view list item.
 *
 * It changes the `listType` of the model `listItem` to a `todo` value.
 * When a _view checkbox element is marked as checked, an additional `todoListChecked="true"` attribute is added to the model item.
 *
 * It is used by {@link @ckEditor5/engine/controller/datacontroller~DataController}.
 *
...
/**
 * A model-to-_view ckeditor5-converter for the `listType` attribute change on the `listItem` model element.
 *
 * This change means that the `<li>` element parent changes to `<ul class="todo-list">` and a
 * {@link @ckEditor5/engine/_view/uielement~UIElement checkbox UI element} is added at the beginning
 * of the list item element (or vice versa).
 *
 * This ckeditor5-converter is preceded by {@link @ckEditor5/list/converters~modelViewChangeType} and followed by
 * {@link @ckEditor5/list/converters~modelViewMergeAfterChangeType} to handle splitting and merging surrounding lists of the same type.
 *
 * It is used by {@link @ckEditor5/engine/controller/editingcontroller~EditingController}.
 *
 * @see module:engine/conversion/downcastdispatcher~DowncastDispatcher#event:attribute
 * @param {Function} onCheckedChange Callback fired after clicking the checkbox UI element.
 * @param {@ckEditor5/engine/_view/_view~View} view Editing _view controller.
 * @returns {Function} Returns a conversion callback.
 */
...
/**
 * A model-to-_view ckeditor5-converter for the `todoListChecked` attribute change on the `listItem` model element.
 *
 * It marks the {@link @ckEditor5/engine/_view/uielement~UIElement checkbox UI element} as checked.
 *
 * It is used by {@link @ckEditor5/engine/controller/editingcontroller~EditingController}.
 *
 * @see module:engine/conversion/downcastdispatcher~DowncastDispatcher#event:attribute
...
/**
 * A model-to-_view position at zero offset mapper.
 *
 * This helper ensures that position inside todo-list in the _view is mapped after the checkbox.
 *
 ...
 *
 * @param {@ckEditor5/engine/_view/_view~View} view
 * @return {Function}
 */
...
//
// @private-attribute
// @param {@ckEditor5/engine/model/item~Item} modelItem
// @param {@ckEditor5/engine/_view/downcastwriter~DowncastWriter} viewWriter
// @param {Boolean} isChecked
// @param {Function} onChange
// @returns {@ckEditor5/_view/uielement~UIElement}
